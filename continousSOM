...
class SOM(object):
    ...
    def __init__(self, m, n, dim, n_iterations=100, alpha=None, sigma=None):
        ...
        def top_left(tensor, pivot):
            ...
            self.pedo = pivot
            self.pedo2 = pivot[0]
            self.pedo3 = 3 #tf.less(m, tf.unpack(pivot[1]))
            print("to int: ", tf.constant([2, 0]))
            #print("pack: ", tf.pack([[pivot[0], 0], [pivot[1], 0]]))
            slice = tf.slice(tensor, pivot, tf.pack([n-pivot[0], m-pivot[1]])) 
            slice = tf.reverse(slice, [True, True])
            slice = tf.pad(slice, tf.constant([[pivot[0], 0], [pivot[1], 0]]))
            return slice
    ...
    def train(self, input_vects):
        ...
        for iter_no in range(self._n_iterations):
            ...
            for input_vect in input_vects:
                _, bmu_index, dist_2, bmu_loc, lr, pedo, pedo2, pedo3 = self._sess.run([self._training_op, self.bmu_index, self.bmu_distance_squares, self.bmu_loc, 
                                                self.learning_rate_op, self.pedo, self.pedo2, self.pedo3],
